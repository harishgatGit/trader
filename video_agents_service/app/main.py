...
from fastapi import FastAPI, BackgroundTasks, Header, HTTPException, Depends, status
...
def find_job_by_id(job_id: str) -> Optional[VideoJob]:
    ...
    if base_dir.exists():
        # Look for job-state.json recursively
...
def trigger_callback(job: VideoJob):
    """Triggers callback notification back to NestJS"""
    payload = {
        "jobId": job.jobId,
...
def run_video_generation_workflow(job_id: str):
    ...
    try:
        ...
        # ----------------------------------------------------
        print(f"[{job.ticker}] Normalizing report narrative...")
        ...
        # ----------------------------------------------------
        # Agent 2: Generate Layman Narration Script
        # ----------------------------------------------------
        ...
        # Audio validation passed (inside voiceover agent)
        ...
        try:
            ...
            if scenes_list:
                # Calculate character count of narration for each scene
                ...
                if total_chars > 0:
                    ...
                    # Adjust the last scene's duration to make the sum exactly audio_duration + 1.5s (safety buffer)
                    ...
                    # Update storyboard total durationSeconds
                    storyboard["durationSeconds"] = sum(float(s.get("durationSeconds", 6)) for s in scenes_list)
                    
                    # Save the updated storyboard JSON back to disk
                    storyboard_path = job_dir / "storyboard.json"
        ...
        # Create default relative video URL served by NestJS proxy
        job.videoUrl = f"/api/video-jobs/{job.jobId}/video"
        job.artifacts.finalVideo = str(video_path.resolve())
        
        # Save placeholder ffprobe.json for callback compatibility
        ...
        # ----------------------------------------------------
        # Agent 6: Video Validation Agent
        # ----------------------------------------------------
...
def create_video_job(request: VideoJobRequest, background_tasks: BackgroundTasks, api_key: str = Depends(get_api_key)):
    """
    Creates a new video job for the stock ticker.
    Runs video generation workflow in the background.
    """
    ticker = request.ticker.upper()
    date_str = request.reportDate
    
    # Check if a successful video already exists for this symbol/date
    existing_job = load_job_from_disk(ticker, date_str)
    if existing_job and existing_job.status == JobStatus.COMPLETED and not request.forceRegenerate:
        # Skip generation as successful video exists
        existing_job.status = JobStatus.SKIPPED_EXISTING_VIDEO
        persist_job(existing_job)
        return existing_job
    ...
    # Save source report to outputs folder first
    job_dir = storage_service.get_job_dir(ticker, date_str)
    source_report_path = job_dir / "source-report.json"
    ...
    background_tasks.add_task(run_video_generation_workflow, job_id)
...
def retry_video_job(jobId: str, background_tasks: BackgroundTasks, api_key: str = Depends(get_api_key)):
    """Retries a failed video job"""
    job = find_job_by_id(jobId)
    if not job:
        raise HTTPException(status_code=404, detail=f"Video job {jobId} not found")
    ...
    # Launch background generation
    background_tasks.add_task(run_video_generation_workflow, jobId)
