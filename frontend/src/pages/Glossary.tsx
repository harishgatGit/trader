...
const glossaryData: GlossaryItem[] = [
  ...
  },
  {
    ...
    hoverBorderClass: 'hover:border-emerald-500/50 dark:hover:border-emerald-500/40',
  },
  {
    ...
    hoverBorderClass: 'hover:border-amber-500/50 dark:hover:border-amber-500/40',
  },
  {
    ...
    badgeColorClass: 'bg-red-500/10 text-red-655 dark:text-red-400 border-red-500/20',
    hoverBorderClass: 'hover:border-red-500/50 dark:hover:border-red-500/40',
  },
  {
    term: 'Sideways Range',
    category: 'Stages',
    ...
    badgeColorClass: 'bg-slate-500/10 text-slate-605 dark:text-slate-400 border-slate-500/20',
    hoverBorderClass: 'hover:border-slate-500/50 dark:hover:border-slate-500/40',
  },
  ...
  },
  {
    ...
    hoverBorderClass: 'hover:border-blue-500/50 dark:hover:border-blue-500/40',
  },
  {
    term: 'Reversal Setup',
    category: 'Setups',
  ...
  },
  {
    ...
    badgeColorClass: 'bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 border-cyan-500/20',
    hoverBorderClass: 'hover:border-cyan-500/50 dark:hover:border-cyan-500/40',
  },
  {
    term: 'VWAP',
    category: 'Indicators',
    ...
    badgeColorClass: 'bg-amber-500/10 text-amber-655 dark:text-amber-400 border-amber-500/20',
    hoverBorderClass: 'hover:border-amber-500/50 dark:hover:border-amber-500/40',
  },
  {
    ...
    badgeColorClass: 'bg-pink-500/10 text-pink-600 dark:text-pink-400 border-pink-500/20',
    hoverBorderClass: 'hover:border-pink-500/50 dark:hover:border-pink-500/40',
  },
  {
    ...
    hoverBorderClass: 'hover:border-teal-500/50 dark:hover:border-teal-500/40',
  },
  {
    term: 'ATR (Volatility)',
    category: 'Indicators',
    ...
    hoverBorderClass: 'hover:border-orange-500/50 dark:hover:border-orange-500/40',
  },
  ...
  },
  {
    ...
    badgeColorClass: 'bg-fuchsia-500/10 text-fuchsia-600 dark:text-fuchsia-400 border-fuchsia-500/20',
    hoverBorderClass: 'hover:border-fuchsia-500/50 dark:hover:border-fuchsia-500/40',
  }
];
  ...
  return (
    <PageContainer>
      ...
      <PageHeader
        ...
        subtitle="Learn technical indicators, setups, and trend cycle stages translated into everyday analogies"
      />

      {/* Control Row (Search & Filter) */}
      <div className="card flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 shadow-sm">
        {/* Search */}
        <div className="relative flex-1 max-w-md">
          ...
          {searchTerm && (
            <button
              ...
              className="absolute right-3 top-3.5 text-slate-450 hover:text-slate-300"
            >
              <X className="h-4 w-4" />
            </button>
          )}
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all duration-200 cursor-pointer ${
                selectedCategory === cat.id
                  ? 'bg-brand-500/10 text-brand-500 dark:text-brand-400 border-brand-500/40 shadow-sm'
                  : 'bg-slate-900/10 dark:bg-slate-900/20 text-slate-450 border-slate-800 hover:text-slate-200 hover:border-slate-400'
              }`}
            >
              {cat.label}
            </button>
      ...
      {filteredItems.length > 0 ? (
        <ResponsiveGrid cols={2}>
          {filteredItems.map((item, idx) => (
            <div
              key={idx}
              className={`card border border-slate-800 transition-all duration-300 flex flex-col justify-between ${item.hoverBorderClass}`}
            >
              <div className="space-y-3">
                {/* Term title and category */}
                <div className="flex items-start justify-between gap-4 border-b border-slate-850/50 pb-2">
                  <h3 className="text-sm font-bold text-slate-100 tracking-tight">
                    {item.term}
                  </h3>
                  <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full border shrink-0 ${item.badgeColorClass}`}>
                    {item.badgeText}
                  </span>
                </div>
        ...
        <div className="flex flex-col items-center justify-center py-16 card border border-dashed border-slate-800">
          <BookOpen className="w-8 h-8 text-slate-650 mb-3" />
          <p className="text-sm font-semibold text-slate-350">No terms found</p>
          <p className="text-xs text-slate-450 mt-1">Try modifying your search query or switching categories</p>
        </div>
      )}
    </PageContainer>
  );
